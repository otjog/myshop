<div class="row no-gutters">
    @foreach( $products_row as $key => $product )
        <div class="product-item-wrap product-item-light col-12 col-lg-4 rounded-0 border-right border-bottom">
                                ...
                                <div class="row">
                                    @if( isset($product->price['sale']) && $product->price['sale'] > 0)
                                        <div class="col-6">
                                            <span class="small">
                                                <s>
                                                    {{$product->price['value'] + $product->price['sale']}}<small>{{$global_data['components']['shop']['currency']['symbol']}}</small>
                                                </s>
                                            </span>
                                        </div>
                                    @endif
                                    <div class="col text-danger">
                                        <span>{{ $product->price['value']}}</span><small>{{$global_data['components']['shop']['currency']['symbol']}}</small>
                                    </div>
                                </div>
                            </div>
                        </div>
